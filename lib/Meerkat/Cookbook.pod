...
documents and that's probably a bad idea.  Either store as epoch seconds or
...
MongoDB will convert them to an internal type when storing, return them as
...
Ultimately, if you want a consistent database field type, your options are
...
* Option 3: store only DateTime(::Tiny) objects and let the attribute coerce them to a Meerkat::DateTime objects

What you B<don't> want to do is mix storing DateTime objects and raw epoch
seconds or any other format.

Note that Meerkat::DateTime objects only work when your data structure has a
C<MeerkatDateTime> type constraint on it.  You can't just store
Meerkat::DateTime objects into any random field or data structure and expect
things to work.
...
But if you update the field, you probably have to pack the replacement object
yourself.
...
This shouldn't happen during C<create> because the data has just been validated
and is being inserted, but it could happen during either C<update> or C<sync>
