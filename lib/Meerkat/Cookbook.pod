...
=head2 Default collection class

You may want to set a default collection class for all your models without
without defining a collection class for each model.

For example one may want to change how C<Meerkat> determines collection names:

   package My::Meerkat::Collection
    
   use Moose;
   extends 'Meerkat::Collection';
    
   # lower case with no namespace
   sub _build_collection_name {
       my $self = shift;
       ( my $name = lc($self->class) ) =~ s/.*?([^:]+)$/$1/;
       return $name;
   }
   ...
   my $meerkat = Meerkat->new(
       model_namespace => "My::Model",
       default_collection_class => 'My::Meerkat::Collection'
   );
   
   $meerkat->collection('Person'); # My::Meerkat::Collection
